...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void intelli_plug_early_suspend(struct early_suspend *h)
{
	if (intelli_plug_active) 
	{
...
static void intelli_plug_late_resume(struct early_suspend *h)
{
	if (intelli_plug_active) 
	{
		intelli_plug_resume();
...
#ifdef CONFIG_HAS_EARLYSUSPEND
    	unregister_early_suspend(&intelli_plug_early_suspend_handler);
#endif
